...
import "../styles/WatchlistDetail.css";
      ...
      const fetchWatchlistAndRatings = async () => {
        try {
          const [watchlistRes, ratingsRes] = await Promise.all([
            axios.get(`http://localhost:9999/watchlist`, {
              params: { userId: user.id },
            }),
            axios.get(`http://localhost:9999/ratings`, {
              params: { userId: user.id },
            }),
          ]);
          const watchlistMovieIds = watchlistRes.data.map(
            (item: { movieId: number }) => item.movieId
          );
          const userRatings = ratingsRes.data.map(
